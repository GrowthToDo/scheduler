...
show_usage() {
    ...
    echo "  2. List all CloudWatch log groups with /aws/lambda/Tt3StagingPR prefix"
...
LOG_GROUP_PREFIX="/aws/lambda/Tt3StagingPR"
...
# Step 2: Get all CloudWatch log groups with Tt3StagingPR prefix
...
for LOG_GROUP in "${LOG_GROUPS_ARRAY[@]}"; do
    if [ -n "$LOG_GROUP" ]; then
        ...
        # Extract PR number from log group name (/aws/lambda/Tt3StagingPR123-* -> 123)
        # Pattern: /aws/lambda/Tt3StagingPR{PR_NUMBER}-*
